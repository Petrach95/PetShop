...
<body>
	...
	<section>
		<h1 id="titolo">I NOSTRI ANIMALI</h1>
			...
			<div class="tbl-content">
				<table cellpadding="0" cellspacing="0" border="0">
					<tbody>
						...
						String tipo = request.getAttribute("tipo")==null? null :(String)request.getAttribute("tipo");
						
						for (Animale a : animaleDao.findAllInVendita()) {
							if (a.getTipo().equals(tipo) || tipo == null){
						%>
						<tr >
							<td><%=a.getId()%></td>
							<td class="immagine" ><img <% out.println("src='img/"+a.getTipo()+".png'");  %>></td>
							<td><%=a.getTipo()%></td>
							<td><%=a.getNomeA()%></td>
							...
							<td>
								<form action="AcquistoServlet" method="post">
									<input name="idAnimale" type="text" value="<%=a.getId()%>" hidden />
									<input type="submit" value="Compra" class="btm btm_td" />
								</form>
							</td>
						</tr>
						<%
